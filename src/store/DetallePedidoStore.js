...
const useDetallePedidoStore = create((set)=>({
    ...
    addDetallePedido: async(detallePedido)=>{
        try {
            const response = await axios.post('https://bakend-bradatec.onrender.com/detallePedido',detallePedido)
    ...
    fetchDetallePedido: async()=>{
        try {
            const response = await axios.get('https://bakend-bradatec.onrender.com/detallePedido')
    ...
    deleteDetallePedido: async(ID_Detalle)=>{
        try {
            const response = await axios.delete(`https://bakend-bradatec.onrender.com/detallePedido/${ID_Detalle}`)
    ...
    updateDetallePedido: async (ID_Detalle, updatedData) => {
        try {  // Realiza una solicitud PUT para actualizar el estudiante en el servidor.
            const response = await axios.put(`https://bakend-bradatec.onrender.com/detallePedido/${ID_Detalle}`, updatedData)
